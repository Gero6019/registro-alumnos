...
router.post("/login",delay,(req,res)=>{
    ...
    if(!name || typeof name !== "string") return res.status(400).json({status:"Error", message:"Credenciales invalidas."});
    if(!password || typeof password !== "string") return res.status(400).json({status:"Error", message:"Credenciales invalidas."});
